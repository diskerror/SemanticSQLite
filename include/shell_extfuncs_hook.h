...
//
// Wired in via CMake: -DSQLITE_SHELL_EXTFUNCS=SEMQLITE -include this header,
// when compiling vendor/sqlite/shell.c. No upstream file is modified.
#ifndef SEMQLITE_SHELL_EXTFUNCS_HOOK_H
#define SEMQLITE_SHELL_EXTFUNCS_HOOK_H

#include <sqlite3.h>

/* Implemented in src/ext_functions.c — registers DMPHON(), EMBEDDING_SIM(),
 * EMBEDDING_DIST() on the given connection. */
extern int semqlite_register(sqlite3 *db, char **pzErrMsg, const void *pApi);

/* Implemented in src/embed.c — registers EMBED(), SEMQLITE_SET(), SEMQLITE_GET()
 * and ensures the semqlite_config table exists on the given connection. */
extern int semqlite_register_embed(sqlite3 *db);

static int semqlite_init_all(sqlite3 *db) {
    int rc = semqlite_register(db, 0, 0);
    if (rc != SQLITE_OK) return rc;
    return semqlite_register_embed(db);
}

#define SEMQLITE_INIT(db)         semqlite_init_all((db))
#define SEMQLITE_EXPOSE(db, msg)  ((void)0)

#endif /* SEMQLITE_SHELL_EXTFUNCS_HOOK_H */
